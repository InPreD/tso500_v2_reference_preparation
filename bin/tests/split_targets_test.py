...
@mark.parametrize(
    ...
    [
        ...
        ),  # all output fragments being of standard_size
        (
            "chr1",
            100,
            350,
            75,
            100,
            [["chr1", "100", "200"], ["chr1", "200", "350"]],
        ),  # the last fragment being larger than standard_size
        (
            "chr1",
            100,
            275,
            50,
            100,
            [["chr1", "100", "200"], ["chr1", "200", "275"]],
        ),  # the last fragment being smaller than standard_size (an "extra fragment")
        (
            "chr1",
            100,
            150,
            ...
            200,
            [["chr1", "100", "150"]],
        ),  # a fragment being smaller than minimum_size (the whole input region being preserved)
    ],
)
def test_fragment_region(chrom, start, end, minimum_size, standard_size, fragment_list):
    assert (
        fragment_region(chrom, start, end, minimum_size, standard_size) == fragment_list
    )
