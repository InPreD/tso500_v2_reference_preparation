...
def fragment_region(
    chrom: str, start: int, end: int, minimum_size: int, standard_size: int
) -> list[list[str]]:
    fragments = []
...
def main():
    ...
    if minimum_size > standard_size:
        logger.error(
            "Please make sure that standard_size is larger than minimum size. Exiting."
        )
        sys.exit(1)

    if (minimum_size < 1) or (standard_size < 1):
        logger.error(
            "Please make sure that the minimum_size and standard_size are larger than 0. Exiting."
        )
        sys.exit(2)
    ...
    with (
        open(input_targets, "r") as input_file,
        ...
        for line in input_file:
            ...
            region_fragments = fragment_region(
                chrom, start, end, minimum_size, standard_size
            )
